...
def main():
    ...
    filename_complete = (
        f"all_tickers_complete_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    all_tickers_complete.to_csv(filename_complete, index=False)
    print(
        f"✅ Saved {len(all_tickers_complete)} complete tickers to: {filename_complete}"
    )
    ...
    usdt_filename = (
        f"usdt_tickers_complete_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
